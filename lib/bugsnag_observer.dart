...
class BugsnagObserver extends RouteObserver<PageRoute<dynamic>> {
  ...
  void _sendScreenView(PageRoute<dynamic> route, {String action = 'On'}) {
    ...
    if (screenName != null) {
      Bugsnag.instance.leaveBreadcrumb('$action $screenName',
          type: BugsnagBreadcrumb.navigation);
    }
  }
  ...
  void didPush(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    if (route is PageRoute) {
      _sendScreenView(route,
          action: 'Pushed from ${previousRoute?.settings.name} to');
    }
  }
  ...
  void didReplace({Route<dynamic>? newRoute, Route<dynamic>? oldRoute}) {
    ...
    if (newRoute is PageRoute) {
      _sendScreenView(newRoute,
          action: 'Replaced ${oldRoute?.settings.name} with');
    }
  }
  ...
  void didPop(Route<dynamic> route, Route<dynamic>? previousRoute) {
    ...
    if (previousRoute is PageRoute && route is PageRoute) {
      _sendScreenView(previousRoute,
          action: 'Popped from ${previousRoute.settings.name} to');
    }
  }
}
